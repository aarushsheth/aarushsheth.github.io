...
[^1]: Test
[^2]: <!-- prettier-ignore --> Recall that in bra-ket notation, we use $| v \rangle$ to denote a vector $v$ in a complex vector space and $\langle f |$ to denote a linear map $f: V \mapsto \mathbb{C}$. The value of $f$,at $v$ is the inner product of the two, denoted $\langle f | v \rangle$ 
